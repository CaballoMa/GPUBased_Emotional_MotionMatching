...
UE::PoseSearch::FSearchResult UPoseSearchDatabase::Search(UE::PoseSearch::FSearchContext& SearchContext, dataInComputeShader& computeShader_input, int dataBaseIndex) const
{
	...
	if (PoseSearchMode == EPoseSearchMode::BruteForce || PoseSearchMode == EPoseSearchMode::PCAKDTree_Compare)
	{
		...
		if (!OutputFromBuffer.IsEmpty())
		{
			...
			int _bestIdx = 0;
			float _bestCost = FLT_MAX;

			CriticalSection.Lock();
			for (int i = 0; i < OutputFromBuffer.Num(); i++)
			{
			...
			int idx = (int)OutputFromBuffer[_bestIdx][2];
			CriticalSection.Unlock();
			const float NotifyAddend = SearchIndex.PoseMetadata[idx].GetCostAddend();
...
void UPoseSearchDatabase::collectingComputeShaderContext(UE::PoseSearch::FSearchContext& SearchContext, dataInComputeShader& computeShader_input, int dataBaseIndex) const
{
	...
	int step = int(floor(pose_sum / POSE_SEARCH_GAP));
	...
	{
#if !GPU_OPEN
		if (CurrTime - LastTime > 0.5f)
		{
			LastTime = CurrTime;
			CriticalSection.Lock();
			OutputFromBufferPtr->Empty();
			CriticalSection.Unlock();
		}
#endif
	...
	//Params.weightsSqrt.Reserve(weightsSqrt.Num() * ((current_pose_max - start_point) / POSE_SEARCH_GAP));
	...
	*localFramePtr += 1;
	...
	if (newAsyncExecution->currFrame >= FRAME_RECORD_LENGTH)
	{
